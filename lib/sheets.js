 ...
 * хранится в отдельной ячейке (L2), НЕ затрагиваемой clearState/setState,
 * чтобы пережить очистку основного состояния позиции. Нужна, чтобы бот не
 * заходил мгновенно повторно в тот же только что закрывшийся сетап. */
...
async function setCooldownUntil(untilDate) {
  const sheets = getSheetsClient();
  await sheets.spreadsheets.values.update({
    spreadsheetId: SHEET_ID(),
    range: "State!L2",
    valueInputOption: "RAW",
    requestBody: { values: [[untilDate.toISOString()]] },
  });
}
...
module.exports = {
  ...
  getCooldownUntil, setCooldownUntil,
};
